...
Date: 2026-03-13
Status: authoritative / covers job, result, meta, queue item, hook payload, review decision, workflow contract, task snapshot
...
`docs/WORKFLOW.md` is human-authored project policy; `state/tasks_snapshot.json`
is a regenerated derived artifact. They still carry explicit version fields so
tooling can reason about compatibility instead of inferring it from filenames.
...
### workflow contract — Human-edited project policy

The workflow contract is stored as Markdown with YAML front matter and validated
by `scripts/validate_artifacts.py --workflow`.

Compatibility rule:
- **Additive optional fields**: safe, as long as loaders ignore unknown fields.
- **Changing required control fields** such as `approval_gates` or `retry_policy`: breaking — requires a contract version bump.
- **Tightening semantics without a schema change**: risky — document explicitly and update validation rules together.

### task snapshot — Rebuildable derived artifact

The task snapshot is generated from task files and can always be regenerated.
This makes compatibility cheaper:

Compatibility rule:
- **Additive fields**: safe, regenerate the snapshot.
- **Breaking layout changes**: bump `snapshot_version` and regenerate instead of migrating by hand.
- **Checksum algorithm change**: breaking for consumers that compare checksums; coordinate version bump with reader changes.
...
| Workflow contract | `contract_version: 1` | `_system/contracts/workflow.schema.json` |
| Task snapshot | `snapshot_version: 1` | generated by `scripts/claw.py task-snapshot` |
