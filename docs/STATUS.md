...
- **Epic 15 (Operator transport/session UX)** — в работе; `TASK-022` закрыт, следующий slice — `TASK-023` про directives/context binding поверх live status feed

## Next

- `TASK-023` — directives/context binding поверх live status feed и task/project context carry-over
- После `TASK-023`: открыть `TASK-024..026` для operator session/file-exchange/transport plugin слоя
