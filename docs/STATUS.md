...
- **TASK-023 закрыт**: добавлен filesystem-backed contract для operator directives через `_system/engine/operator_context.py` и `claw openclaw bind-context`; precedence `reply_context > directive > default`, `ctx:` round-trip и branch→worktree binding покрыты `tests/operator_context_test.sh`
...
- **Budget/governance guardrails закрыты**: workflow contract получил `guardrails` policy, worker и `claw run --execute` пишут `guardrail_snapshot.json`, project snapshot живёт в `state/guardrails/budget_snapshot.json`, а hard-stop/approval path переиспользует существующие `state/approvals/`
...
- **Native subagent starter packs добавлены**: repo-scoped `.codex/config.toml`, `.codex/agents/*`, `.claude/agents/*` и template-scoped starter packs теперь позволяют использовать Codex/Claude subagents без ручного bootstrap
- **Onboarding guidance добавлен**: `README.md`, `AGENTS.md`, `CLAUDE.md` и `docs/SUBAGENTS.md` фиксируют, когда использовать subagents, Claude agent teams и сам `claw`, плюс рекомендуемый first-run chat flow для выбора доступных CLI
- **Shared session files для Claude/Codex handoff закрыты**: task-scoped `state/session_docs/<TASK>/manifest.json` + `files/`, CLI `session-files|session-file-put|session-file-fetch`, schema validation и `shared_files` summary в `session-status`
...
- **Epic 14 (PaperClip-inspired coordination primitives)** — закрыт; `TASK-017..021` завершены, coordination primitives теперь включают wakes, claims, sessions, org graph и budget/governance guardrails
- **Epic 15 (Operator transport/session UX)** — закрыт; `TASK-022..026` завершены, operator surface теперь покрывает status/context/session/file-exchange/plugin checks

## Next

- Разделить `tests/run_all.sh` на быстрый и полный прогоны, чтобы `run-checks`/workflow contract не гоняли весь suite рекурсивно и чтобы unit/integration прогоны были разделены явно
...
| 2026-03-15 | TASK-020 post-review hardening: project overrides + live delegation policy | `_system/engine/org_graph.py`, `tests/org_graph_test.sh`, `docs/{PLAN.md,STATUS.md}` | `bash tests/org_graph_test.sh`; `bash tests/run_all.sh` | ✅ partial `docs/ORG_GRAPH.yaml` overrides now merge with base agent metadata instead of replacing it, and `delegation.allow_self_delegate` is enforced from file-backed policy; assumption: self-delegation stays opt-in and scoped by explicit project policy | TASK-021 |
| 2026-03-16 | TASK-021 budget and governance guardrails | `_system/engine/{workflow_contract.py,budget_guardrails.py,__init__.py}`, `_system/contracts/workflow.schema.json`, `scripts/claw.py`, `tests/{budget_guardrails_test.sh,workflow_contract_test.sh,run_all.sh}`, `projects/{_template,demo-project}/docs/WORKFLOW.md`, `projects/_claw-dev/tasks/TASK-021.md`, `docs/{PLAN.md,STATUS.md}` | `bash tests/budget_guardrails_test.sh`; `bash tests/workflow_contract_test.sh`; `bash tests/run_all.sh` | ✅ file-backed run/project guardrail snapshots, deterministic soft/hard budget semantics, governance approval gating via existing approval artifacts, and status/dashboard/openclaw visibility implemented; assumption: sensitive config changes are inferred from backticked spec paths and configured governance risk flags | TASK-022 |
...
| 2026-03-16 | TASK-022 live status feed for operators | `scripts/claw.py`, `tests/{live_status_feed_test.sh,openclaw_test.sh,queue_cli_test.sh,run_all.sh}`, `projects/_claw-dev/tasks/TASK-022.md`, `projects/_claw-dev/state/{tasks_snapshot.json,workflow_graph.json}`, `docs/{PLAN.md,STATUS.md}` | `python3 -m py_compile scripts/claw.py`; `bash tests/live_status_feed_test.sh`; `bash tests/openclaw_test.sh`; `bash tests/queue_cli_test.sh`; `bash tests/run_all.sh`; `python3 scripts/claw.py task-snapshot projects/_claw-dev` | ✅ добавлен единый polling-friendly live status feed для `claw status` и `openclaw summary`: queue/run state, current step, stream tail, event snapshot, graceful fallback на битых `meta/event_snapshot/events`; assumption: `run_status` в feed нормализован для оператора (`queued/running/awaiting_approval/success/...`), а сырой `meta.json` статус сохранён отдельно в `meta_status` | TASK-023 |
| 2026-03-16 | TASK-023 message directives and context binding | `_system/engine/{__init__.py,operator_context.py}`, `scripts/claw.py`, `tests/{operator_context_test.sh,openclaw_test.sh,live_status_feed_test.sh,run_all.sh}`, `projects/_claw-dev/tasks/TASK-023.md`, `projects/_claw-dev/state/{tasks_snapshot.json,workflow_graph.json}`, `docs/{PLAN.md,STATUS.md}` | `python3 -m py_compile scripts/claw.py _system/engine/operator_context.py`; `bash tests/operator_context_test.sh`; `bash tests/openclaw_test.sh`; `bash tests/live_status_feed_test.sh`; `bash tests/run_all.sh`; `python3 scripts/claw.py task-snapshot projects/_claw-dev` | ✅ добавлен `claw openclaw bind-context` с нормализованным parsing/binding для `/agent`, `/project`, `@branch` и `ctx:` footer; precedence фиксирован как `reply_context > directive > default`, unknown/conflicting context падает явно, а branch target детерминированно требует worktree-backed execution (`git_worktree`, либо сохраняет `isolated_checkout`); assumption: `ctx:` footer остаётся компактным single-line transport contract (`ctx: project=… agent=… branch=…`) без multiline/YAML режима | TASK-024 |
| 2026-03-16 | TASK-024 operator session memory and resume handles | `_system/engine/{__init__.py,operator_session_store.py}`, `_system/contracts/operator_session_state.schema.json`, `_system/registry/agents.yaml`, `scripts/{claw.py,validate_artifacts.py}`, `tests/{operator_session_test.sh,operator_context_test.sh,openclaw_test.sh,session_state_test.sh,run_all.sh}`, `projects/_claw-dev/tasks/TASK-024.md`, `docs/{PLAN.md,STATUS.md,contracts.md}` | `python3 -m py_compile scripts/claw.py _system/engine/operator_session_store.py`; `bash tests/operator_session_test.sh`; `bash tests/operator_context_test.sh`; `bash tests/openclaw_test.sh`; `bash tests/session_state_test.sh`; `bash tests/run_all.sh` | ✅ добавлен repo-scoped operator session store в `state/operator_sessions/` с provider-neutral handles, derived `resume_line` через registry template, `openclaw session-status|session-update|session-reset|session-new-thread`, и auto-resume/continuation resolution в `openclaw bind-context`; assumption: operator session artifacts живут на уровне repo scope+engine, а project/branch остаются binding metadata и suppress auto-resume при reply/directive mismatch | TASK-025 |
| 2026-03-16 | TASK-025 safe file exchange for project roots | `_system/engine/{__init__.py,file_exchange.py}`, `scripts/claw.py`, `tests/{file_exchange_test.sh,openclaw_test.sh,operator_context_test.sh,review_batch_cli_test.sh,run_all.sh}`, `projects/_claw-dev/tasks/TASK-025.md`, `projects/_claw-dev/state/{tasks_snapshot.json,workflow_graph.json}`, `docs/{PLAN.md,STATUS.md}` | `python3 -m py_compile scripts/claw.py _system/engine/file_exchange.py`; `bash tests/file_exchange_test.sh`; `bash tests/operator_context_test.sh`; `bash tests/openclaw_test.sh`; `bash tests/review_batch_cli_test.sh`; `bash tests/run_all.sh`; `python3 scripts/claw.py task-snapshot projects/_claw-dev`; `python3 scripts/claw.py workflow-graph projects/_claw-dev` | ✅ добавлены `openclaw file-put|file-fetch` поверх reusable file exchange contract: относительные пути нормализуются и не могут выйти из project/worktree root, deny-globs режут чувствительные paths, uploads пишутся атомарно, каталоги fetch-ятся как zip, а worktree-targeted exchange требует явный `--run`; assumption: transport передаёт bind-context payload и конкретный run только когда действительно нужен materialized workspace | TASK-026 |
| 2026-03-16 | TASK-026 transport plugin surface and setup checks | `_system/engine/{__init__.py,error_codes.py,file_exchange.py,operator_transport.py,transport_plugins/{__init__.py,file_exchange.py}}`, `_system/registry/operator_transports.yaml`, `scripts/claw.py`, `tests/{transport_plugins_test.sh,file_exchange_test.sh,openclaw_test.sh,run_all.sh}`, `projects/_claw-dev/{tasks/TASK-026.md,state/{tasks_snapshot.json,workflow_graph.json}}`, `docs/{PLAN.md,BACKLOG.md,STATUS.md}` | `python3 -m py_compile scripts/claw.py _system/engine/file_exchange.py _system/engine/operator_transport.py _system/engine/transport_plugins/file_exchange.py`; `bash tests/transport_plugins_test.sh`; `bash tests/file_exchange_test.sh`; `bash tests/openclaw_test.sh`; `bash tests/run_all.sh`; `python3 scripts/claw.py task-snapshot projects/_claw-dev`; `python3 scripts/claw.py workflow-graph projects/_claw-dev` | ✅ добавлен explicit transport registry с lazy provider loading, built-in `file_exchange` backend переведён на plugin contract, появились `openclaw transports` и `openclaw doctor`, а setup checks теперь детерминированно ловят invalid ids/config, duplicate providers, missing binaries и unsupported workspace combinations; assumption: transport extensibility пока ограничена narrow backend/provider contract без динамического генератора новых CLI surfaces | split `tests/run_all.sh` into fast/full lanes |
| 2026-03-17 | native subagent starter packs + first-run onboarding docs | `.codex/{config.toml,agents/*}`, `.claude/agents/*`, `projects/_template/.{codex,claude}/**`, `README.md`, `AGENTS.md`, `CLAUDE.md`, `docs/{SUBAGENTS.md,PLAN.md,BACKLOG.md,STATUS.md}`, `tests/{foundation_scaffold_test.sh,import_project_test.sh,workflow_contract_test.sh,run_all.sh}` | `bash tests/foundation_scaffold_test.sh`; `bash tests/import_project_test.sh`; `bash tests/workflow_contract_test.sh`; `bash tests/run_all.sh` | ✅ repo и template теперь сразу готовы к Codex/Claude subagents; decision rules и chat-first onboarding flow задокументированы; assumption: auth/subscription проверяется через chat confirmation + cheap capability probe, а не через brittle hidden heuristics | дальше можно добавить doctor/checks для CLI availability при первом запуске |
| 2026-03-18 | TASK-027 shared session files for cross-agent handoff | `_system/engine/{__init__.py,session_docs.py}`, `_system/contracts/session_docs_manifest.schema.json`, `scripts/{claw.py,validate_artifacts.py}`, `tests/{session_docs_test.sh,session_state_test.sh,file_exchange_test.sh,run_all.sh}`, `projects/_claw-dev/{tasks/TASK-027.md,specs/SPEC-027.md,state/{tasks_snapshot.json,workflow_graph.json}}`, `docs/{PLAN.md,BACKLOG.md,STATUS.md,contracts.md}` | `python3 -m py_compile scripts/claw.py _system/engine/session_docs.py scripts/validate_artifacts.py`; `bash tests/session_docs_test.sh`; `bash tests/session_state_test.sh`; `bash tests/file_exchange_test.sh`; `bash tests/run_all.sh`; `python3 scripts/claw.py task-snapshot projects/_claw-dev`; `python3 scripts/claw.py workflow-graph projects/_claw-dev` | ✅ добавлен task-scoped handoff layer `state/session_docs/<TASK>/` с manifest metadata (`author`, `note`, `sha256`, `bytes`), CLI `session-files|session-file-put|session-file-fetch`, schema validation и `shared_files` summary в `session-status`; assumption: deep research / PRD остаются обычными project files под git, а `session_docs` используются как узкий cross-agent handoff channel | split `tests/run_all.sh` into fast/full lanes |
| 2026-03-18 | agent instructions for session_docs handoff | `AGENTS.md`, `CLAUDE.md`, `docs/STATUS.md` | `bash tests/run_all.sh` | ✅ добавлены явные инструкции для агентов: в начале задачи проверять `session-files`, читать релевантные handoff notes через `session-file-fetch` и перед передачей/остановкой писать краткий handoff через `session-file-put`; assumption: policy задаётся инструкциями в repo-level docs, без автоматического принуждения в runtime | дальше можно встроить это в prompt template/build_run |
| 2026-03-19 | runtime hygiene cleanup after agent pass | `_system/contracts/job.schema.json`, `scripts/{build_run.py,claw.py,create_project.sh,execute_job.py,generate_review_batch.py,hooklib.py}`, `tests/{launch_plan_test.sh,run_all.sh,task_to_job_test.sh}`, `scripts/run_task.py`, `projects/_claw-dev/state/{tasks_snapshot.json,workflow_graph.json}`, `docs/STATUS.md` | `bash tests/run_all.sh`; `bash tests/command_registry_test.sh`; `bash tests/task_graph_lint_test.sh`; `bash tests/decision_log_test.sh`; `bash tests/workflow_graph_artifact_test.sh`; `bash tests/event_replay_test.sh`; `bash tests/decompose_epic_test.sh`; `bash tests/workflow_enforcement_test.sh`; `bash tests/epic_status_test.sh`; `bash tests/launch_plan_test.sh` | ✅ узкие runtime-fixes подтверждены: `_claw-dev` slug разрешён в schema, общие helper-функции вынесены в `hooklib.py`, `launch_plan_test.sh` включён в общий suite, dashboard/review/orchestrate cleanup отрегрессирован; assumption: maintenance slice не привязан к отдельному `TASK-*`, поэтому зафиксирован только в live journal | split `tests/run_all.sh` into fast/full lanes |
