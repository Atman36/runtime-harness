...
- **Epic 15 (Operator transport/session UX)** — в работе; `TASK-022..025` закрыты, следующий slice — `TASK-026` про transport plugin/setup checks

## Next

- `TASK-026` — transport plugin surface + setup/doctor checks поверх уже закрытых status/context/session/file-exchange primitives
- После `TASK-026`: разделить `tests/run_all.sh` на быстрый и полный прогоны, чтобы workflow contract не гонял весь suite рекурсивно
