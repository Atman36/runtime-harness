...
|------|-----------|---------------------|
| `job.schema.json` | `runs/**/job.json` | `job_version`, `run_id`, `project`, `preferred_agent`, `task`, `spec`, `artifacts` |
| `result.schema.json` | `runs/**/result.json` | `run_id`, `status` |
| `meta.schema.json` | `runs/**/meta.json` | `run_id`, `status`, `project` |
...
- **`result.json`** starts sparse (`status: pending`, `agent`) and gains
  `started_at`, `exit_code`, `summary`, `hook`, etc. as the run progresses.
...
- **`meta.json`** similarly starts at `status: created` and accumulates
  `executor`, `hook`, timestamps after execution.
...
themselves. They do not touch `scripts/claw.py` or `_system/engine/`.
