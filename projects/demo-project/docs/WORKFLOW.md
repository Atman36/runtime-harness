...
- `guardrails.budget`: Coarse run-cost accounting with optional warning/hard-stop
  thresholds backed by run/project snapshots on disk.
- `guardrails.governance`: Approval-required triggers for risk flags, sensitive
  paths, workspace modes or specific agents; approval continues through the
  existing `state/approvals/` flow.
